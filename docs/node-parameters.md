...
**If it's wrong.** The failure here is the quiet one. A mailbox nothing is delivered to keeps the connection up and reports healthy, which looks exactly like no mail arriving — there is no error to see, because nothing has gone wrong.

So if this is on and never fires, suspect the mailbox name before suspecting the connection. The check is the read-mail job itself: run it by hand against the same mailbox, and if it finds the message the watcher should have caught it, while if it finds nothing either then the mail is somewhere else.

A name that does not exist is the honest failure — the connection fails to open, says so, and retries with a backoff.
...
**What it does.** The name shown beside the address on mail rn sends — "Pieter <you@example.com>" rather than "you@example.com" on its own. It is set here and nowhere else: the address comes from the account board and the two are combined per message.

Empty is the default and means no name, which is what every send has done so far.

**Why you would change it.** It is the only setting in this group a stranger ever sees. The rest decide whether a connection works; this one decides what is in the From column of somebody's mail client, beside a subject line, before they have opened anything — and a bare address is the shape of mail people have been taught to be careful with.

It is also on the short list of settings that cannot be corrected afterwards. Changing it fixes the next send and nothing already in a mailbox, which is the same reason the tracker's base URL is checked before a link is minted rather than after.

**If it's wrong.** Nothing fails. An empty value sends the address alone, exactly as before, and any value at all is passed to the mail library as a name rather than pasted into a header — so a comma, a quote or an accent is encoded correctly instead of splitting the field.
