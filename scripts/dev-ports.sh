...
#
#   pane      PORT   API    hooks  tracker
#   ~/rn      1790   3010   3011   3012
#   ~/ca      1791   3020   3021   3022
#   ~/cb      1792   3030   3031   3032
#   ~/cc      1793   3040   3041   3042
#
# Ten apart rather than one, so the API and the two listeners beside it never
# land on a neighbour's set. The hooks port is the one a tunnel points at, and
# pointing a tunnel at another worktree's backend is not a mistake worth making
# reachable — which goes double for the tracker, whose links outlive the send.
#
...
# The click tracker — see docs/link-tracking.md. Its own port rather than a
# route on the hooks listener, so that listener keeps the one-route property
# docs/network.md §4 and docs/sec.md both argue from.
export BACKEND_TRACKER_PORT="${BACKEND_TRACKER_PORT:-$(( BACKEND_PORT + 2 ))}"
